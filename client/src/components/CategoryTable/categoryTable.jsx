    ...
    return (
        <div>
            <div className="mt-4 pt-20 mb-4 flex justify-center">
                <button
                    className="inline-block px-6 h-11 py-2 text-xs font-medium leading-6 text-center text-white uppercase transition bg-green-500 rounded shadow ripple hover:shadow-lg hover:bg-green-600 focus:outline-none">
                    <a href="/postCategory">
                        Create category
                    </a>
                </button>
                <div className="relative mr-6 my-2 ml-2 -mt-0.5">
                    <input type="search" className="bg-purple-white shadow rounded border-0 p-3" value={input.name} onChange={(e) => handleChange(e)} placeholder="Search by name...   " />
                ...
                <tbody>
                    {categoryArray && categoryArray.length > 0 ? categoryArray.map((c, id) => {
                        return <tr key={id} className="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                            ...
                            <td className="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
                                <button className="h-10 px-5 m-2 text-blue-100 transition-colors duration-150 bg-blue-600 rounded-lg focus:shadow-outline hover:bg-blue-700"> <Link to={'/editCategory/' + c._id}>✍</Link> </button>
